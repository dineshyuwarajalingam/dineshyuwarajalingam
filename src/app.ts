...
async function createUser(member) {
  ...
  const userResponse = await axios.post(`${baseUri}/users`, member, {
    auth: {
      username,
      password,
    },
  ...
  const cardResponse = await axios.post(`${baseUri}/cards`, cardPayload, {
    auth: {
      username,
      password,
    },
...
app.post('/register-user', async (req, res) => {
  try {
    ...
    res.status(201).json({
      message: 'User registered with Marqeta successfully',
      data: userData,
    });
  } catch (error) {
    console.error(
      'Error registering user with Marqeta:',
      error.response?.data || error.message
    );
    res.status(500).json({
      error: 'Failed to register user with Marqeta',
